 ...
 * Returns a multiplier in [0.3, 1.7]; amplitude scales with uncertainty
 * (1 - confidence). With reducedMotion, returns ~1 (static star).
 */
export function breathingFlicker(
  ...
  const t = nowMs / 1000
  const slow = 0.5 + 0.5 * Math.sin(t * 0.72 + seed * 0.13)
  const slower = 0.5 + 0.5 * Math.sin(t * 0.21 + seed * 0.77)
